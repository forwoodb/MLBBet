...
# # df_pitchers[['Date']] = month_days 
# df_pitchers[['Name']] = names
# df_pitchers[['ERA']] = eras
# df_pitchers[['FIP']] = fips

# # df_pitchers = pd.merge(df_games, df_pitchers, on=['Name','Date'])
...
for month in range(4,11): 
    ...
    for day in range(start_day,end_day):
        if day < 10:
            if month < 10:
                url = "https://www.fangraphs.com/leaders.aspx?pos=all&stats=pit&lg=all&qual=" + min_ip + "&type=8&season=" + year + "&month=1000&season1=" + year + "&ind=0&team=&rost=&age=&filter=&players=&startdate=" + year + "-03-01&enddate=" + year + "-0" + str(month) + "-0" + str(day) + "&page=1_5000"
            else:
                url = "https://www.fangraphs.com/leaders.aspx?pos=all&stats=pit&lg=all&qual=" + min_ip + "&type=8&season=" + year + "&month=1000&season1=" + year + "&ind=0&team=&rost=&age=&filter=&players=&startdate=" + year + "-03-01&enddate=" + year + "-" + str(month) + "-0" + str(day) + "&page=1_5000"
            print(url)
        else:
            url = "https://www.fangraphs.com/leaders.aspx?pos=all&stats=pit&lg=all&qual=" + min_ip + "&type=8&season=" + year + "&month=1000&season1=" + year + "&ind=0&team=&rost=&age=&filter=&players=&startdate=" + year + "-03-01&enddate=" + year + "-0" + str(month) + "-" + str(day) + "&page=1_5000"
        ...
        for row in rows:
            ...
            eras.append([era.text.strip()])
            fips.append([fip.text.strip()])

            # # trim space?
            # try:
            #     fips.append([fip.text])
            # except:
            #     if fip == '':
            #         pass
            # # print(fip.text)
        ...
        df_pitchers.to_csv(
            # '/Applications/MAMP/htdocs/Python/Django/DataApps/MLBWeb/csv_data/06-18-22/rosters.csv'
            '/Applications/MAMP/htdocs/Python/WebScraping/MLB/csv_data/pitchers/' + year + '/' + str(month) + '-' + str(day) + '-' + abb_year + '.csv',
            index=False
        )
